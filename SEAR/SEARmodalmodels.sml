...
Binarymap.insert (!psyms,p,tl)

lookup_pred (!psyms) "MCOMPACT"
*)
...
(rpt strip_tac >> 
 ...
 ‘ENT(PEC(f),f)’
 >-- (strip_tac >>
     drule Thm_6_24 >> first_x_assum drule >> pop_assum strip_assume_tac >>
...
(form_goal “
 (!A fs:mem(Pow(form(A))).
  ...
  ?W M:mem(Pow(W * W) * Exp(W,Pow(A))) w. 
   SATIS(M,w,fs)) ==>
 !A f:mem(form(A)). PUS(f) ==> ?f0. PE(f0) & EQV(f,f0)”))

val Thm_6_25_iff = prove_store("Thm_6_25_iff",
e0
(rpt strip_tac >> dimp_tac >> disch_tac 
 >-- (drule Thm_6_25_l2r >> first_x_assum drule  >> arw[]) >>
...
val TRUTH_def = qdefine_psym("TRUTH",[]) ‘T’
