...
(rpt strip_tac >>
 ...
 >-- (rw[GSYM IN_EXT_iff,Empty_def] >>
     ...
     >-- (first_x_assum irule >>
         rw[hatclass_def] >> rw[GSYM IN_NONEMPTY] >>
         fs[IN_Inter] >> strip_tac (* 2 *)
         >-- (qexists_tac ‘x''’ >> arw[]) >>
         qexists_tac ‘C’ >> arw[]) >>
     fs[Diff_def]) >>
 qby_tac
...
(rpt strip_tac >>
 ...
 >-- (rw[IN_Inter,IN_BIGUNION] >> 
     ...
     >-- (first_x_assum irule >> rw[hatclass_def] >>
         rw[GSYM IN_NONEMPTY] >> strip_tac (* 2 *)
         >-- (qexists_tac ‘x’ >> arw[]) >> 
         qexists_tac ‘C’ >> rw[]) >>
     qexists_tac ‘k’ >> arw[]) >>
 fs[antisym_def] >>
...
(rpt strip_tac >>
 ...
 >-- (fs[ischoice_def] >>
     first_x_assum irule >> 
     rw[hatclass_def] >> arw[] >> qexists_tac ‘k’ >> rw[]) >>
 rpt strip_tac (* 2 *)
 ...
 >-- (fs[ischoice_def] >>
     first_x_assum irule >> 
     rw[hatclass_def] >> drule Inter_both_NONEMPTY >>
     arw[] >>
     qexists_tac ‘C’ >> rw[]) >>
 qby_tac ‘SS(C,k)’ >--
...
(rpt strip_tac >>
 ...
 >-- (irule ischoice_ex >> 
     rw[NOT_EMPTY] >> fs[GSYM IN_NONEMPTY] >>
     rw[hatclass_def] >>
     qexists_tac ‘a’ >> rw[]) >>
 pop_assum strip_assume_tac >>
